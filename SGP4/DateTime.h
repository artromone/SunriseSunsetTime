...
int SGP4_DECL clock_gettime(int, struct timespec *tv);
#endif // _MSC_VER
...
public:
     ...
     */
	static DateTime Now( bool microseconds = false ) noexcept
	{
		const auto sinceEpoch = std::chrono::system_clock::now().time_since_epoch();
		const auto ticks = microseconds
		    ? std::chrono::duration_cast< std::chrono::microseconds >( sinceEpoch ).count() * TicksPerMicrosecond
		    : std::chrono::duration_cast< std::chrono::seconds >( sinceEpoch ).count() * TicksPerSecond;
...
inline SGP4::DateTime operator+( const DateTime& dt, TimeSpan ts )
{
    return SGP4::DateTime( dt.Ticks() + ts.Ticks() );
}

inline SGP4::DateTime operator-( const DateTime& dt, const TimeSpan& ts )
{
    return SGP4::DateTime( dt.Ticks() - ts.Ticks() );
}

inline SGP4::TimeSpan operator-( const DateTime& dt1, const DateTime& dt2 )
{
    return SGP4::TimeSpan( dt1.Ticks() - dt2.Ticks() );
}
